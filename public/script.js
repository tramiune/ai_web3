...
const SERVICE_PACKAGES = [
    { id: 'plus', name: 'Plus', cost: MODEL_COST_LEGACY_FAST, featureKeys: ['services.plus_f1', 'services.plus_f2', 'services.plus_f3'], featured: true },
    { id: 'viral', name: 'Viral', cost: MODEL_COST_LEGACY_TURBO, featureKeys: ['services.viral_f1', 'services.viral_f2', 'services.viral_f3'] }
];
...
window.selectTemplate = async (id, url) => {
    ...
    if (dur != null && dur > maxSec + 0.15) {
        showToast(t('modals.video_will_trim_on_submit', { sec: maxSec }));
    }
    updateFirstOrderUI();
};
...
async function trimVideoBlobToMaxSec(blob, maxSec = MAX_VIDEO_DURATION_SEC, options = {}) {
    let duration;
    try {
...
async function applyTikTokVideoFromUrl(pageUrl, options = {}) {
    ...
    if (needsTrim && !prefersServerSideTrim()) {
        try {
            const trimmed = await trimVideoBlobToMaxSec(blob, tiktokMaxSec, { onProgress });
            blob = trimmed.blob;
        } catch (trimErr) {
        ...
        onProgress?.('server_trim', { sec: tiktokMaxSec });
    }
        ...
        const { trimmed } = await applyTikTokVideoFromUrl(pageUrl, {
            onProgress: (phase, params) => {
                if (phase === 'trimming' && btn) {
                    btn.textContent = t('modals.video_trimming', params || { sec: tiktokMaxSec });
                }
                if (phase === 'trimming') {
                    showToast(t('modals.video_trimming', params || { sec: tiktokMaxSec }));
                }
            }
        });
        showToast(trimmed
            ? t('modals.tiktok_fetch_trimmed', { sec: tiktokMaxSec })
...
async function setupEventListeners() {
    ...
    document.querySelectorAll('input[name="model-type"]').forEach(radio => {
        radio.addEventListener('change', () => {
            ...
            const fileInput = document.getElementById('file-video');
            const existing = fileInput?.files?.[0];
            if (existing?.type?.startsWith('video/') && window.currentVideoSource === 'upload') {
                renderVideoFilePreview('preview-video-container', existing, {
                    inputId: 'file-video',
                    changeKey: 'modals.video_change',
                    maxDurationSec: getMaxVideoSecForSelectedModel()
        ...
        orderForm.addEventListener('submit', async (e) => {
            ...
            try {
                        ...
                        const result = await applyTikTokVideoFromUrl(tiktokUrl, {
                            onProgress: (phase, params) => {
                                if (phase === 'loading_ffmpeg') {
                                    ...
                                    updateVideoTrimOverlay('modals.video_trimming', params || { sec: tiktokMaxSec });
                                } else if (phase === 'fetching') {
                                    updateVideoTrimOverlay('modals.tiktok_fetch_on_submit');
                                }
                            }
                ...
                if (videoFile) {
                    ...
                    kalingDurationSec = await getVideoDurationFromUrl(templateUrl);
                }
                ...
                if (charFile.size > MAX_CHAR_FILE_BYTES) {
                    resetOrderSubmitUi(submitBtn, progressDiv);
                    return showToast(t('modals.char_size_limit'));
                }
                ...
                updateVideoTrimOverlay('modals.order_uploading_char');
...
async function runBatchChannelOnFrontend() {
    const channelUrl = document.getElementById('batch-channel-url')?.value?.trim() || '';
    const username = parseTikTokUsername(channelUrl);
    if (!username) {
        showToast(t('build_channel.status_need_channel'));
    ...
    let templateImageUrl = '';
    const configId = getBatchChannelConfigId();
    const { db, doc, getDoc, setDoc, runTransaction, collection, serverTimestamp } = window.firebase;
    try {
        const existing = await getDoc(doc(db, 'batchChannelConfig', configId));
        if (existing.exists()) {
            templateImageUrl = existing.data().templateImageUrl || '';
    ...
    if (!charFile && !templateImageUrl) {
        showToast(t('build_channel.status_need_template'));
        return;
    }

    const mainBtn = document.getElementById('batch-channel-main-btn');
    if (mainBtn) {
        mainBtn.disabled = true;
        mainBtn.innerText = 'Đang xử lý...';
    }
    ...
    if (chip) {
        chip.className = 'batch-status-chip batch-status-chip--running';
        chip.textContent = 'Đang quét...';
    }
    if (phaseEl) {
        phaseEl.textContent = 'Đang quét kênh TikTok...';
    ...
    try {
        if (charFile) {
            phaseEl.textContent = 'Đang tải lên ảnh nhân vật...';
            templateImageUrl = await uploadFile(charFile, 'characters');
        }
        ...
        const payload = await channelRes.json();
        if (payload?.code !== 0 || !payload?.data) {
            throw new Error(payload?.msg || 'Kênh không tồn tại hoặc không có video.');
        }

        const rawVideos = payload.data.videos || [];
        if (!rawVideos.length) {
            throw new Error('Kênh này hiện tại chưa đăng video nào.');
        }

        const nowUtc = new Date();
        const nowVn = new Date(nowUtc.getTime() + 7 * 60 * 60 * 1000);
        const yestVn = new Date(nowVn);
        yestVn.setDate(nowVn.getDate() - 1);
        const startYest = new Date(yestVn.getFullYear(), yestVn.getMonth(), yestVn.getDate(), 0, 0, 0);
        const endYest = new Date(yestVn.getFullYear(), yestVn.getMonth(), yestVn.getDate(), 23, 59, 59);
        const startTs = Math.floor((startYest.getTime() - 7 * 60 * 60 * 1000) / 1000);
        const endTs = Math.floor((endYest.getTime() - 7 * 60 * 60 * 1000) / 1000);

        const yesterdayVideos = rawVideos.filter(v => {
            const ts = Number(v.create_time || 0);
            return ts >= startTs && ts <= endTs;
        });

        if (!yesterdayVideos.length) {
            throw new Error('Không tìm thấy video nào đăng ngày hôm qua trên kênh này.');
        }

        const limitCount = getBatchYesterdayVideoCount();
        let targetVideos = yesterdayVideos;
        if (limitCount > 0) {
            targetVideos = yesterdayVideos.slice(0, limitCount);
        }

        if (chip) chip.textContent = 'Đang tạo đơn...';

        const batchModel = getBatchModel();
        const modelCost = batchModel.cost;

        for (let i = 0; i < targetVideos.length; i++) {
            const v = targetVideos[i];
            const vid = String(v.video_id || v.id || '');
            const progressLabel = `Video ${i + 1}/${targetVideos.length}`;
            if (statsEl) {
                statsEl.innerHTML = `<span class="batch-run-stat">${t('build_channel.status_run_videos', { count: targetVideos.length })}</span>` +
                                     `<span class="batch-run-stat">${t('build_channel.status_run_orders', { count: ordersCreated })}</span>`;
            }

            try {
                const userRef = doc(db, "users", currentUser.uid);
                const userSnapCheck = await getDoc(userRef);
                const userData = userSnapCheck.data();
                const coins = normalizeCoins(userData?.coins);
                if (coins < modelCost) {
                    throw new Error('Tài khoản không đủ coin để tạo đơn tiếp theo.');
                }

                const playUrl = v.hdplay || v.play || v.wmplay;
                if (!playUrl) {
                    throw new Error('Không tìm thấy link tải của video này.');
                }

                phaseEl.textContent = `${progressLabel}: Đang tải video từ TikTok. Vui lòng không đóng trình duyệt...`;
                const proxyUrl = `/api/media-download?url=${encodeURIComponent(playUrl)}`;
                const videoRes = await fetch(proxyUrl);
                if (!videoRes.ok) {
                    throw new Error('Không thể tải video từ proxy.');
                }
                let blob = await videoRes.blob();

                if (blob.size > MAX_VIDEO_FILE_BYTES) {
                    throw new Error('Kích thước video TikTok vượt quá 50MB.');
                }

                let duration = Number(v.duration || v.video_duration || 0);
                if (!isFinite(duration) || duration <= 0) {
                    try {
                        duration = await getBlobVideoDurationSec(blob);
                    } catch (_) {
                        duration = batchModel.maxVideoSec + 1;
                    }
                }

                if (duration > batchModel.maxVideoSec + 0.15) {
                    phaseEl.textContent = `${progressLabel}: Đang cắt video còn ${batchModel.maxVideoSec} giây...`;
                    const trimmed = await trimVideoBlobToMaxSec(blob, batchModel.maxVideoSec);
                    blob = trimmed.blob;
                }

                phaseEl.textContent = `${progressLabel}: Đang tải video lên server...`;
                const uploadedVideoUrl = await uploadFile(new File([blob], 'tiktok_video.mp4', { type: 'video/mp4' }), 'motions');

                phaseEl.textContent = `${progressLabel}: Đang lưu đơn hàng...`;
                await runTransaction(db, async (transaction) => {
                    const freshUserSnap = await transaction.get(userRef);
                    const freshUser = freshUserSnap.data();
                    const currentCoins = normalizeCoins(freshUser?.coins);
                    if (currentCoins < modelCost) {
                        throw new Error('Coins');
                    }

                    transaction.update(userRef, {
                        coins: currentCoins - modelCost,
                        updatedAt: serverTimestamp()
                    });
                    ...
                    transaction.set(orderRef, {
                        ...
                        userEmail: currentUser.email,
                        userName: currentUser.displayName,
                        packageName: batchModel.serviceLabel,
                        modelId: batchModel.modelId,
                        renderProvider: batchModel.renderProvider || "aidancing",
                        maxVideoSec: batchModel.maxVideoSec || 15,
                        vaeDurationSec: batchModel.vaeDurationSec || 10,
                        vaeResolution: batchModel.vaeResolution || "720p",
                        serviceType: "auto_video",
                        serviceLabel: "Tạo video tự động",
                        costCoins: modelCost,
                        clientVersion: APP_CLIENT_VERSION,
                        characterImageLink: templateImageUrl,
                        referenceVideoLink: uploadedVideoUrl,
                        aspectRatio: "9:16",
                        status: "pending",
                        resultLink: "",
                        adminNote: "",
                ...
                console.error(`[BatchOrder] Failed for video ${vid}:`, err);
                const errMsg = err.message === 'Coins' ? 'Tài khoản không đủ coin.' : (err.message || 'Lỗi xử lý.');
                errors.push(`Video ${vid}: ${errMsg}`);
            }
        }
        ...
        await setDoc(doc(db, 'batchChannelConfig', configId), {
            ...
            channelUsername: username,
            templateImageUrl,
            batchModel: getBatchModelKey(),
            yesterdayVideoCount: limitCount,
            updatedAt: serverTimestamp()
        }, { merge: true });

        if (chip) {
            chip.className = errors.length === targetVideos.length ? 'batch-status-chip batch-status-chip--fail' : 'batch-status-chip batch-status-chip--done';
            chip.textContent = errors.length === targetVideos.length ? 'Thất bại' : 'Hoàn tất';
        }
        phaseEl.textContent = `Hoàn tất! Đã tạo thành công ${ordersCreated}/${targetVideos.length} đơn hàng.`;
        if (errors.length > 0) {
            if (errorsEl) {
        ...
        showToast(`Tạo thành công ${ordersCreated} đơn hàng tự động!`);
        setTimeout(() => {
            closeModal('batch-channel-modal');
            window.loadMoreMyOrders?.(true);
        }, 3000);

    } catch (e) {
        console.error('[BatchRun] Fatal error:', e);
        if (chip) {
            chip.className = 'batch-status-chip batch-status-chip--fail';
            chip.textContent = 'Lỗi';
        }
        phaseEl.textContent = `Lỗi hệ thống: ${e.message || e}`;
        showToast(e.message || 'Lỗi hệ thống trong lúc cào và tạo video.');
    } finally {
        if (mainBtn) {
            mainBtn.disabled = false;
            mainBtn.innerText = 'Tạo video ngay';
        }
    }
}

window.handleBatchChannelMainAction = async () => {
    await runBatchChannelOnFrontend();
};
